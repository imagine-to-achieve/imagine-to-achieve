...
def get_ur5_fk_solution(joint_angles):
    joint_angles = np.asarray(joint_angles)
    if joint_angles.shape[0] < 6:
        raise ValueError(f"UR5 FK expects at least 6 joint angles, got {joint_angles.shape}")
    ...
    dh_params = [[0.0, 0.089159, np.pi / 2, joint_angles[0]],
                 [-0.425, 0.0, 0.0, joint_angles[1]],
                 [-0.39225, 0.0, 0.0, joint_angles[2]],
                 [0.0, 0.10915, np.pi / 2, joint_angles[3]],
                 [0.0, 0.09465, -np.pi / 2, joint_angles[4]],
                 [0.0, 0.0823, 0.0, joint_angles[5]]]

    T = np.eye(4)
    for a, d, alpha, theta in dh_params:
        T = T @ get_tf_mat(a, d, alpha, theta)
    return T
...
def get_fk_solution(joint_angles):
    joint_angles = np.asarray(joint_angles)
    if joint_angles.shape[0] == 6:
        return get_ur5_fk_solution(joint_angles)
    if joint_angles.shape[0] >= 7:
        return get_franka_fk_solution(joint_angles)
    raise ValueError(f"FK expects 6 UR5 joints or 7 Franka joints, got {joint_angles.shape}")

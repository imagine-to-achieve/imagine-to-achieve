...
def compute_grpo_actor_loss_fn(**kwargs) -> tuple[torch.Tensor, dict]:
    ...
    """
    lambda_action = float(kwargs.get("lambda_action", 1.0))
    actor_loss_unscaled, actor_metrics_data = compute_ppo_actor_loss(**kwargs)
    lambda_action_tensor = actor_loss_unscaled.new_tensor(lambda_action)
    actor_loss = actor_loss_unscaled * lambda_action_tensor

    metrics_data = {
        "actor/action_policy_loss": actor_loss_unscaled.detach(),
        "actor/lambda_action": lambda_action_tensor.detach(),
    }
    metrics_data.update(actor_metrics_data)
    metrics_data["actor/policy_loss"] = actor_loss.detach()
    metrics_data["actor/action_ratio"] = actor_metrics_data["actor/ratio"]
    metrics_data["actor/action_logprob"] = masked_mean(
        kwargs["logprobs"].detach(), kwargs.get("loss_mask", None)
    )
    metrics_data["actor/action_clip_fraction"] = actor_metrics_data[
        "actor/clip_fraction"
    ]

...
def preprocess_embodied_advantages_inputs(
    ...
    **kwargs,
    ...
    """
    kwargs.pop("dynamic_gammas", None)
    latent_motion = kwargs.get("latent_motion", None)
    if kwargs["reward_type"] == "chunk_level":
        ...
        if latent_motion is not None:
            latent_motion = latent_motion.mean(dim=-1, keepdim=True)
    ...
    if latent_motion is not None:
        latent_motion = latent_motion.transpose(1, 2).reshape(n_steps, bsz)
...
def calculate_episode_scores(
    ...
    """Collapse embodied rewards to one score per trajectory without grouping."""
    if rewards.ndim != 2:
        raise ValueError(f"Expected rewards [T,B], got {tuple(rewards.shape)}")
    if dones.ndim != 2 or dones.shape[1] != rewards.shape[1]:
        raise ValueError(
            "Expected dones [T+1,B] with matching batch size, got "
            f"rewards={tuple(rewards.shape)}, dones={tuple(dones.shape)}"
        )
    if dones.shape[0] < rewards.shape[0] + 1:
        raise ValueError(
            "dones must contain the initial boundary plus one entry per reward step"
        )
    scores = torch.zeros(
        rewards.shape[1],
        dtype=rewards.dtype,

...
def trim_cpu_allocator() -> bool:
    """Best-effort release of free glibc heap pages back to the OS."""
    try:
...
def clear_memory(sync=True, trim_cpu=False, collect_ipc=False):
    if sync:
        Worker.torch_platform.synchronize()
    gc.collect()
    Worker.torch_platform.empty_cache()
    if collect_ipc:
        ipc_collect = getattr(Worker.torch_platform, "ipc_collect", None)

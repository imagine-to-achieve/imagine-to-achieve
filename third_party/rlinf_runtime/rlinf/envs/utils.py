...
def put_text_on_image(
    ...
    """Put readable text lines on an image with automatic line wrapping."""
    assert image.dtype == np.uint8, image.dtype
    image = Image.fromarray(image.copy())
    draw = ImageDraw.Draw(image, "RGBA")
    font_size = max(18, min(34, round(image.height / 18)))
    font = ImageFont.load_default(size=font_size)
    margin = max(8, round(font_size * 0.45))
    line_gap = max(4, round(font_size * 0.25))
    wrap_width = max(max_width, image.width - 2 * margin)
    ...
    for line in lines:
        ...
        for word in words:
            ...
            if font.getlength(test_line) <= wrap_width or not current_line:
    ...
    metrics = [draw.textbbox((0, 0), text=line, font=font) for line in new_lines]
    text_width = max(bbox[2] - bbox[0] for bbox in metrics)
    text_height = sum(bbox[3] - bbox[1] for bbox in metrics) + line_gap * (len(new_lines) - 1)
    pad = max(4, margin // 2)
    box_x = max(0, int(origin[0])) if origin is not None else margin // 2
    box_y = max(0, int(origin[1])) if origin is not None else margin // 2
    box = (
        box_x,
        box_y,
        min(image.width - margin // 2, box_x + text_width + 2 * pad),
        min(image.height - margin // 2, box_y + text_height + 2 * pad),
    )
    draw.rounded_rectangle(box, radius=max(3, margin // 2), fill=(0, 0, 0, 190))

    text_x = box_x + pad
    y = box_y + pad
    for line, bbox in zip(new_lines, metrics):
        textheight = bbox[3] - bbox[1]
        draw.text((text_x, y), text=line, font=font, fill=(255, 255, 255, 255))
        y += textheight + line_gap
    return np.array(image)

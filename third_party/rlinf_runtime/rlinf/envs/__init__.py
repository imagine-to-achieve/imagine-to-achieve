...
class SupportedEnvType(Enum):
    ...
    COSMOS_FORWARD_DYNAMICS_WM = "cosmos_forward_dynamics_wm"
    COSMOSSELFCTRLWORLD = "cosmos_self_ctrl_world"
...
def get_env_cls(env_type: str, env_cfg=None):
    ...
    if env_type == SupportedEnvType.MANISKILL:
        ...
        return CtrlWorldEnv
    elif env_type == SupportedEnvType.COSMOS_FORWARD_DYNAMICS_WM:
        from rlinf.envs.world_model.world_model_cosmos_forward_dynamics_env import (
            CosmosForwardDynamicsEnv,
        )

        return CosmosForwardDynamicsEnv
    elif env_type == SupportedEnvType.COSMOSSELFCTRLWORLD:
        from rlinf.envs.world_model.world_model_ctrl_world_env import (
            CosmosSelfFeedbackCtrlWorldEnv,
        )

        return CosmosSelfFeedbackCtrlWorldEnv
    else:

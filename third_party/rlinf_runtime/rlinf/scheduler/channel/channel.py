...
class Channel:
    ...
    **Features**:
    ...
    def is_distributed(self) -> bool:
        """Whether this channel has one queue worker replica per cluster node."""
        return self._distributed

    def node_local_key(self, namespace: Any = DEFAULT_KEY) -> tuple[Any, int]:
        """Return a stable queue key scoped to the caller's cluster node.

        Distributed channels place a new key on the node from which that key
        is first used. Including the node rank in the key lets producers and
        consumers that are colocated on a node route large payloads through
        that node's ChannelWorker instead of concentrating every payload on
        rank 0.
        """
        if not self.is_distributed:
            raise RuntimeError(
                "node_local_key() requires a distributed Channel."
            )
        node_rank = self._get_src_node_rank()
        if node_rank < 0:
            raise RuntimeError(
                "Cannot derive a node-local Channel key outside a cluster worker."
            )
        return namespace, int(node_rank)

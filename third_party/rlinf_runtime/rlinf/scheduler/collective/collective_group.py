...
class CollectiveGroupOptions:
    ...
    disable_same_device_ipc: bool = False
    """Stage accelerator tensors through CPU/Gloo when both workers share a
    device. This is intended for very large, infrequent transfers such as
    Actor-to-Rollout full-weight synchronization, where CUDA IPC exporter and
    importer mappings can otherwise remain resident across updates."""
        ...
        while True:
            ...
            with self._stream_ctx:
                ...
                try:
                    ...
                    # Do not let one failed operation kill the daemon queue and
                    # leave every later caller waiting forever. Surface the
                    # original exception through the work's Future instead.
                    work.set_exception(exc)
                    self._logger.exception(
                        "Async %s ID %s failed",
                        "send"
                        if self._type == CollectiveWorkQueue.SEND
                        else "recv",
                        comm_id,
                    )
                else:
                    ...
                    # Release payload references even after a failed operation.
                    work = None
...
class CollectiveGroup:
    ...
    def __init__(
        ...
        self._lock = threading.Lock()
        self._disable_same_device_ipc = False
    ...
    def _init_process_group(
        ...
        with self._lock:
            ...
            self._disable_same_device_ipc = bool(
                options is not None and options.disable_same_device_ipc
            )
            if self._disable_same_device_ipc:
                self._logger.info(
                    "Same-device CUDA IPC is disabled for collective group %s; "
                    "accelerator tensors will be staged through CPU/Gloo",
            ...
            try:
                timeout_minutes = int(
                    Cluster.get_sys_env_var(ClusterEnvVar.TIMEOUT, "180")
                )
            except ValueError as exc:
                raise ValueError(
                    "Invalid TIMEOUT value. It should be an integer representing minutes."
                ) from exc
            store_timeout = timedelta(minutes=timeout_minutes)

            if self._rank == 0:
                # Bind before publishing the port. The old find-free-port then
                # bind sequence had a TOCTOU window in which Ray or another
                # process could claim the selected port and crash this queue
                # with EADDRINUSE. TCPStore(port=0) asks the kernel to choose
                # and bind a port atomically.
                rendezvous_store = dist.TCPStore(
                    host_name=self._group_info.master_addr,
                    port=0,
                ...
                master_port = rendezvous_store.port
    ...
    def _send_cuda_tensor_list_via_cpu(
        self,
        tensors: list[torch.Tensor],
        comm_id: int,
    ) -> None:
        """Stream same-device accelerator tensors through CPU/Gloo.

        A tensor-at-a-time staging buffer keeps host-memory use bounded by the
        largest parameter instead of materializing another full state dict.
        The outer async API already runs this method in its communication
        worker thread, so each low-level transfer is intentionally synchronous
        to keep the temporary CPU tensor alive until Gloo is finished with it.
        """
        self._logger.info(
            "Sending %d accelerator tensors via CPU staging from worker %s",
            len(tensors),
    ...
    def _recv_cuda_tensor_list_via_cpu(
        self,
        tensors: list[torch.Tensor],
        comm_id: int,
    ) -> list[torch.Tensor]:
        """Receive CPU/Gloo staging buffers and copy them to accelerator."""
        self._logger.info(
            "Receiving %d accelerator tensors via CPU staging in worker %s",
            len(tensors),
            self._cur_worker_address.get_name(),
        )
        for tensor in tensors:
            ...
            tensor.copy_(cpu_tensor, non_blocking=False)
            del cpu_tensor
        return tensors
    ...
    def _send_tensor_list(
        ...
        if accel_tensors:
            # Handle CUDA tensor sending with IPC if the peer worker is on the same device
            check_cuda_device_result = self._check_same_device_with_peer()
            if self._disable_same_device_ipc and check_cuda_device_result >= 0:
                self._send_cuda_tensor_list_via_cpu(accel_tensors, comm_id)
            elif check_cuda_device_result == 0:
    ...
    def _recv_tensor_list(self, comm_id: int) -> tuple[list[torch.Tensor], Any]:
        ...
        if has_accel_tensor:
            check_cuda_device_result = self._check_same_device_with_peer()
            if self._disable_same_device_ipc and check_cuda_device_result >= 0:
                received_accel_tensors = self._recv_cuda_tensor_list_via_cpu(
                    [tensor for _, tensor, _ in accel_entries], comm_id
                )

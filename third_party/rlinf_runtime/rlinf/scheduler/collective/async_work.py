...
class AsyncWork:
    ...
    def release(self) -> None:
        """Release references owned by a completed work, when supported."""
...
class AsyncFuncWork(AsyncWork):
    ...
    def set_exception(self, exc: Exception) -> None:
        """Complete this work with an exception if it is still pending."""
        if not self._done.done():
    ...
    def release(self) -> None:
        """Break completed callback chains and drop payload/result references."""
        ...
        self._next_work = None
        self._cuda_event = None
        if isinstance(next_work, AsyncFuncWork):
            next_work.release()
...
class AsyncChannelCommWork(AsyncWork):
    ...
    def release(self) -> None:
        """Drop a consumed Channel payload and its callback/Future cycle.

        Channel receives chain a collective ``AsyncFuncWork`` to the bound
        ``_store_channel_data`` callback. For multi-GiB CPU tensors, leaving
        that completed chain to cyclic GC can keep the Future result and its
        shared-memory storage mapped across an entire subsequent RL update.
        Call this after the received value has been copied/merged elsewhere.
        """
        with AsyncChannelCommWork.store_lock:
            AsyncChannelCommWork.channel_data_store.pop(self._query_id, None)
        async_comm_work = self._async_comm_work
        self._async_comm_work = None
        self._data_future = None
        self._channel_actor = None
        release = getattr(async_comm_work, "release", None)
        if callable(release):
            release()

...
class FSDPStrategyBase(ABC):
    ...
    def create(
        ...
        - native       -> NativeParallelStrategy (model parallelizes itself; RLinf
                           only builds the optimizer/grad-clip/offload on top)
        ...
        match strategy:
            ...
            case FSDPVersion.NATIVE:
                from .native import NativeParallelStrategy

                return NativeParallelStrategy(
                    cfg=cfg,
                    world_size=world_size,
                    dp_group=dp_group,
                    logger=logger,
            ...
            case _:
                raise ValueError(
                    f"Unknown FSDP strategy '{strategy}'. Expected one of: 'fsdp','fsdp2','native'"
                )
    ...
    def save_checkpoint(
        ...
        if save_full_model_weights:
            ...
            # Rank 0 materializes a full CPU state dict (about 11 GiB for
            # Cosmos3). Drop it explicitly instead of leaving its allocator
            # pages resident until a later Python collection cycle.

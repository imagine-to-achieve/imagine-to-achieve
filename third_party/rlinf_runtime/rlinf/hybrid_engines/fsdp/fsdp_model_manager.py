...
class FSDPModelManager:
    ...
    def __init__(self, cfg: DictConfig, world_size: int, rank: int) -> None:
        ...
        if not torch.distributed.is_initialized():
            # create_device_mesh -> init_device_mesh is the first thing to
            # initialize torch.distributed for this actor process (RLinf's
            # own Worker class only sets MASTER_ADDR/PORT/RANK/WORLD_SIZE
            # env vars, it never calls init_process_group itself). Left to
            # its own default, init_device_mesh would pick a plain "nccl"
            # backend for a cuda device_type. That is fine for GPU tensor
            # collectives (FSDP's own all-gather/reduce-scatter), but
            # checkpoint loading (torch.distributed.checkpoint's
            # gather_object, used by DCP's load-plan rendezvous) collects
            # CPU-side pickled bytes, and NCCL's handling of CPU-tensor
            # collectives is fragile once ranks span multiple physical
            # nodes -- observed as an unreproducible-looking "NCCL Error 2:
            # unhandled system error" specifically inside dcp.load()'s
            # gather_object on the first-ever 2-node cross-rank GRPO run.
            # The hybrid backend spec routes CPU tensors to Gloo (robust,
            # TCP-based) and CUDA tensors to NCCL under the same default
            # process group, with no per-call changes needed anywhere else
            # (including the read-only external cosmos-framework's dcp.load
            # call site, which doesn't accept a process_group override).
    ...
    def load_checkpoint(self, load_path: str) -> None:
        ...
        # DCP checkpoints contain optimizer param-group and scheduler state,
        # including the historical learning rate. A resumed run may
        # intentionally override that value (for example, a controlled LR
        # A/B); make the runtime configuration authoritative after restore.
        configured_lr = self._cfg.optim.get("lr", None)
            ...
            if self.lr_scheduler is not None:
                if hasattr(self.lr_scheduler, "base_lrs"):
                    self.lr_scheduler.base_lrs = [
                        configured_lr for _ in self.optimizer.param_groups
                    ]
    ...
    def save_checkpoint(self, save_path: str, step: int = 0) -> None:
        ...
        try:
            self._strategy.save_checkpoint(
                ...
                self.lr_scheduler,
                save_path,
                save_full_model_weights=bool(
                    self._cfg.get("save_full_model_weights", True)
                ),
            )
        finally:
            # Checkpointing is a temporary onload. Preserve the manager entry
            # state so a per-step checkpoint cannot leave Actor model or
            # optimizer tensors resident on the GPUs during the next rollout.
            if restore_optimizer_offload:
                self.offload_optimizer()
            if restore_weight_offload:

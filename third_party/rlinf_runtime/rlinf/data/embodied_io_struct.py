...
class EnvOutput:
    ...
    latent_motion: Optional[torch.Tensor] = None  # [B]
    reset_state_ids: Optional[torch.Tensor] = None  # [B]
    ...
    def __post_init__(self):
        ...
        self.latent_motion = (
            self.latent_motion.cpu().contiguous()
            if self.latent_motion is not None
            else None
        )
    ...
    def prepare_observations(self, obs: dict[str, Any]) -> dict[str, Any]:
        ...
        # Optional full-resolution reset views are an atomic Cosmos policy
        # input contract. Preserve them through EnvOutput serialization only
        # when the environment supplies them, so legacy observation schemas
        # and the 10D pipeline remain unchanged.
        for key in (
            "policy_composite_images",
            "policy_main_images",
            "policy_wrist_images",
    ...
    def merge_env_outputs(env_outputs: list[dict]) -> dict[str, Any]:
        ...
        merged_latent_motion = _merge_optional_tensor_field("latent_motion")
        merged_reset_state_ids = _merge_optional_tensor_field("reset_state_ids")
        ...
        return EnvOutput(
            ...
            latent_motion=merged_latent_motion,
            reset_state_ids=merged_reset_state_ids,
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        env_output_dict["latent_motion"] = self.latent_motion
...
class ChunkStepResult:
    ...
    video_similarity_rewards: torch.Tensor = None  # [B, action_chunk]
    terminal_goal_rewards: torch.Tensor = None  # [B, action_chunk]
    continuous_combined_rewards: torch.Tensor = None  # [B, action_chunk]
    reward_model_probabilities: torch.Tensor = None  # [B, action_chunk]
    dynamic_gammas: torch.Tensor = None  # [B, 1]
    latent_motion: torch.Tensor = None  # [B, 1]
    reset_state_ids: torch.Tensor = None  # [B, 1]
    rollout_uid: torch.Tensor = None  # [B, 1]
    global_group_id: torch.Tensor = None  # [B, 1]
    group_member_id: torch.Tensor = None  # [B, 1]
    source_env_rank: torch.Tensor = None  # [B, 1]
    local_env_id: torch.Tensor = None  # [B, 1]
    reset_seed: torch.Tensor = None  # [B, 1]
    vision_noise_seed: torch.Tensor = None  # [B, 1]
    action_noise_seed: torch.Tensor = None  # [B, 1]
    ctrl_world_noise_seed: torch.Tensor = None  # [B, 1]
    update_id: torch.Tensor = None  # [B, 1]
    logical_round_id: torch.Tensor = None  # [B, 1]
    physical_wave_id: torch.Tensor = None  # [B, 1]
    group_slot: torch.Tensor = None  # [B, 1]
    chunk_id: torch.Tensor = None  # [B, 1]
    reset_episode: torch.Tensor = None  # [B, 1]
    seed_nonce: torch.Tensor = None  # [B, 1]
    shuffle_seed: torch.Tensor = None  # [B, 1]
    retry_count: torch.Tensor = None  # [B, 1]
    forward_inputs: dict[str, torch.Tensor] = field(default_factory=dict)
    versions: torch.Tensor = None  # [B, 1]
    ...
    def __post_init__(self):
        ...
        if self.latent_motion is not None:
            self.latent_motion = self.latent_motion.cpu().contiguous()
        if self.reset_state_ids is not None:
            self.reset_state_ids = self.reset_state_ids.cpu().contiguous()
        for field_name in (
            "rollout_uid",
            "global_group_id",
            "group_member_id",
            ...
            value = getattr(self, field_name)
            if value is not None:
                setattr(self, field_name, value.cpu().to(torch.int64).contiguous())
        if self.forward_inputs:
            self.forward_inputs = put_tensor_device(self.forward_inputs, "cpu")
        if self.versions is not None:
            self.versions = self.versions.cpu().contiguous()
...
class Trajectory:
    ...
    video_similarity_rewards: torch.Tensor = None
    terminal_goal_rewards: torch.Tensor = None
    continuous_combined_rewards: torch.Tensor = None
    reward_model_probabilities: torch.Tensor = None
    dynamic_gammas: torch.Tensor = None
    latent_motion: torch.Tensor = None
    reset_state_ids: torch.Tensor = None
    rollout_uid: torch.Tensor = None
    global_group_id: torch.Tensor = None
    group_member_id: torch.Tensor = None
    source_env_rank: torch.Tensor = None
    local_env_id: torch.Tensor = None
    reset_seed: torch.Tensor = None
    vision_noise_seed: torch.Tensor = None
    action_noise_seed: torch.Tensor = None
    ctrl_world_noise_seed: torch.Tensor = None
    update_id: torch.Tensor = None
    logical_round_id: torch.Tensor = None
    physical_wave_id: torch.Tensor = None
    group_slot: torch.Tensor = None
    chunk_id: torch.Tensor = None
    reset_episode: torch.Tensor = None
    seed_nonce: torch.Tensor = None
    shuffle_seed: torch.Tensor = None
    retry_count: torch.Tensor = None
    libero_verified_success: torch.Tensor = None
    libero_verified_weight: torch.Tensor = None
    ...
    def extract_intervene_traj(self, mode="any"):
        ...
        for i in range(mask.shape[1]):
            ...
            latent_motion = apply_mask(self.latent_motion, i)
            reset_state_ids = apply_mask(self.reset_state_ids, i)
            libero_verified_success = apply_mask(self.libero_verified_success, i)
            libero_verified_weight = apply_mask(self.libero_verified_weight, i)
            ...
            filtered_trajectories.append(
                Trajectory(
                    ...
                    libero_verified_success=libero_verified_success,
                    libero_verified_weight=libero_verified_weight,
    ...
    def extract_success_traj(self, truncate_after_first_success: bool = True):
        ...
        for i in range(batch_size):
            ...
            latent_motion = apply_prefix_mask(self.latent_motion, i, keep_mask)
            reset_state_ids = apply_prefix_mask(self.reset_state_ids, i, keep_mask)
            libero_verified_success = apply_prefix_mask(
                self.libero_verified_success, i, keep_mask
            )
            libero_verified_weight = apply_prefix_mask(
                self.libero_verified_weight, i, keep_mask
            )
...
class EmbodiedRolloutResult:
    ...
    video_similarity_rewards: list[torch.Tensor] = field(default_factory=list)
    terminal_goal_rewards: list[torch.Tensor] = field(default_factory=list)
    continuous_combined_rewards: list[torch.Tensor] = field(default_factory=list)
    reward_model_probabilities: list[torch.Tensor] = field(default_factory=list)
    dynamic_gammas: list[torch.Tensor] = field(default_factory=list)  # trajectory_length
    latent_motion: list[torch.Tensor] = field(default_factory=list)  # trajectory_length
    reset_state_ids: list[torch.Tensor] = field(default_factory=list)  # trajectory_length
    rollout_uid: list[torch.Tensor] = field(default_factory=list)
    global_group_id: list[torch.Tensor] = field(default_factory=list)
    group_member_id: list[torch.Tensor] = field(default_factory=list)
    source_env_rank: list[torch.Tensor] = field(default_factory=list)
    local_env_id: list[torch.Tensor] = field(default_factory=list)
    reset_seed: list[torch.Tensor] = field(default_factory=list)
    vision_noise_seed: list[torch.Tensor] = field(default_factory=list)
    action_noise_seed: list[torch.Tensor] = field(default_factory=list)
    ctrl_world_noise_seed: list[torch.Tensor] = field(default_factory=list)
    update_id: list[torch.Tensor] = field(default_factory=list)
    logical_round_id: list[torch.Tensor] = field(default_factory=list)
    physical_wave_id: list[torch.Tensor] = field(default_factory=list)
    group_slot: list[torch.Tensor] = field(default_factory=list)
    chunk_id: list[torch.Tensor] = field(default_factory=list)
    reset_episode: list[torch.Tensor] = field(default_factory=list)
    seed_nonce: list[torch.Tensor] = field(default_factory=list)
    shuffle_seed: list[torch.Tensor] = field(default_factory=list)
    retry_count: list[torch.Tensor] = field(default_factory=list)
    ...
    libero_verified_success: Optional[torch.Tensor] = None
    libero_verified_weight: Optional[torch.Tensor] = None
    ...
    def append_step_result(self, result: ChunkStepResult):
        if result.actions is not None:
            ...
            if result.reset_state_ids is not None:
                reset_state_ids = result.reset_state_ids
                if reset_state_ids.dim() == 1:
                    reset_state_ids = reset_state_ids.unsqueeze(-1)
                self.reset_state_ids.append(reset_state_ids)
        ...
        if result.latent_motion is not None:
            self.latent_motion.append(result.latent_motion)
        for field_name in (
            "rollout_uid",
            "global_group_id",
            "group_member_id",
    ...
    def append_transitions(self, curr_obs=None, next_obs=None):
        ...
        # These fields are rollout-time policy inputs/metadata rather than
        # persistent environment state. In particular, Ctrl-World exposes the
        # full-resolution policy_* views only on reset; retaining them would
        # make observation keys differ between the first and later chunks and
        # break trajectory stacking.
        transient_obs_keys = (
            "task_descriptions",
            "policy_composite_images",
            "policy_main_images",
    ...
    def to_trajectory(self) -> Trajectory:
        ...
        if len(self.latent_motion) > 0:
            trajectory.latent_motion = (
                torch.stack(self.latent_motion, dim=0).cpu().contiguous()
            )
        if len(self.reset_state_ids) > 0:
            trajectory.reset_state_ids = (
        ...
        for field_name in (
            ...
            if values:
                setattr(
                    trajectory,
                    field_name,
                    torch.stack(values, dim=0).cpu().to(torch.int64).contiguous(),
                )
        if self.libero_verified_success is not None:
            trajectory.libero_verified_success = (
                self.libero_verified_success.cpu().contiguous()
            )
        if self.libero_verified_weight is not None:
            trajectory.libero_verified_weight = (
                self.libero_verified_weight.cpu().contiguous()
            )

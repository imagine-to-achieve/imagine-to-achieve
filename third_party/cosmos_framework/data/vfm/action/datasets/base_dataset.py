...
_TASK_TEXT_KEYS = ("task", "text", "__index_level_0__")
...
def _task_text_from_row(row: dict[str, Any]) -> str:
    for key in _TASK_TEXT_KEYS:
        ...
        if value is not None:
            return str(value)
    raise KeyError(f"Could not find task text in tasks.parquet row; tried keys {_TASK_TEXT_KEYS}, row keys={list(row)}")
...
class ActionBaseDataset(ABC, Dataset):
    ...
    def __init__(
        ...
        self._tasks = {
            int(row["task_index"]): _task_text_from_row(row)
